...
class FRCNN(object):
    ...
    def show_anchors(self, image):
        ...
        #绘制先验框
        box_xy,box_wh=(anchors[:,0:2]+anchors[:,2:4])/2,anchors[:,2:4]-anchors[:,0:2]
        anchors[:,:4]=self.bbox_util.frcnn_correct_boxes(box_xy, box_wh, input_shape, image_shape)
        thickness   =max((np.shape(image)[0]+np.shape(image)[1])//input_shape[0],1)
         
        #绘制9个
        for i,c in list(enumerate(anchors[:9])):
            box=anchors[i]
    ...
    def show_proposals(self, image,count=20):
        ...
        #绘制建议框
        box_xy,box_wh=(rpn_results[0][:,0:2]+rpn_results[0][:,2:4])/2,rpn_results[0][:,2:4]-rpn_results[0][:,0:2]
        rpn_results[0][:,:4]=self.bbox_util.frcnn_correct_boxes(box_xy, box_wh, input_shape, image_shape)
        top_boxes =rpn_results[0][:,:4]
        thickness   =max((np.shape(image)[0]+np.shape(image)[1])//input_shape[0],1)
        
        for i,c in list(enumerate(top_boxes[0:count])):
            box=top_boxes[i]
